...
-(void) initialSetup{
    ...
    contentSprite = [[CCSprite alloc] initWithFile:@"MB_RContent.png"];
    scrollView = [[MBScrollView alloc] initWithSprite:contentSprite];
    scrollView.position = ccp(0, -winSize.height);
    [self addChild:scrollView];
    
    topBarSprite = [[CCSprite alloc] initWithFile:@"MB_RTopBar.png"];
    topBarSprite.position = ccp(winSize.width/2, winSize.height + 50);
    [self addChild:topBarSprite];
    
    backButton = [CCMenuItemImage itemWithNormalImage:@"MB_RBackArrow.png" selectedImage:@"MB_RBackArrow.png" target:self selector:@selector(backPressed)];
