...
-(void) initialSetup{
    ...
    contentSprite = [[CCSprite alloc] initWithFile:@"MB_KContent.png"];
    scrollView = [[MBScrollView alloc] initWithSprite:contentSprite];
    scrollView.position = ccp(0, -winSize.height);
    [self addChild:scrollView];
    
    topBarSprite = [[CCSprite alloc] initWithFile:@"MB_KTopBar.png"];
    topBarSprite.position = ccp(winSize.width/2, winSize.height + 50);
    [self addChild:topBarSprite];
    
    backButton = [CCMenuItemImage itemWithNormalImage:@"MB_KBackArrow.png" selectedImage:@"MB_KBackArrow.png" target:self selector:@selector(backPressed)];
